...
### Edição 0.17 — 2026-08-13 · Dezenove jornadas do protocolo: a conversa acontecendo

- **O livro ganhou um gênero novo**, em `livro/jornadas/`. Os capítulos explicam o que cada peça do padrão é; as jornadas mostram **quem fala com quem, em que ordem, e o que atravessa o fio**. São o vizinho das *journeys* de produto da metodologia Maestro, com uma troca: onde a journey tem fluxograma e captura de tela, a jornada de protocolo tem **diagrama de sequência e tabela de trocas** — e a tabela é a fonte normativa, por acessibilidade medida.
- **Seis decisões de forma, todas em ADR** (0008 a 0013), cada uma consolidando pareceres de especialistas: o esqueleto e a regra "nenhuma seta sem lastro"; o comando de interface como perna de efeito e não canal paralelo; diagrama de estados só com lastro, um por laboratório; a divisão entre as três recusas; a aplicação federada como **procedência de desenho** e nunca laboratório; e a jornada **sem fio**, que a admissão federada obrigou a admitir.
- **A série encontrou vinte e duas lacunas na norma e oito derivas do Anexo A**, e era para encontrar. As mais duras: o requisito da máquina de estados manda as transições fora da tabela falharem, e **a norma não publica tabela nenhuma**; a confirmação é fechada em três campos, então a decisão de um lote de oito é byte a byte idêntica à de um alvo único; o schema do catálogo **rejeita** o campo de atomicidade que o requisito de lote manda declarar; e o pedido estruturado de dados não tem só um tipo de evento faltando — falta também o **corpo de volta**, e essa é a metade mais grave.
- **Três achados que mudam o que o livro afirmava.** O laboratório A tem **dois** caminhos mutadores, com códigos de erro diferentes para a mesma situação. Três das cinco recusas da confirmação **não escrevem evento nenhum** — o que o usuário vê é fabricado no cliente, com sequência zero —, o que contraria o requisito de traço. E a política de reversibilidade do laboratório B, que o capítulo 06 apresentava como estando no caminho, é **código morto**.
- **O achado mais desconfortável**: no laboratório A, propostas vivem em memória de processo e os eventos vão para o banco. Depois de um reinício, o replay reconstrói o cartão pendente com botões e a confirmação falha. É **pior que perder o gate** — a interface mente com convicção. A lição de protocolo: o replay reconstrói a *conversa*, não o *estado*.
- **O bloco federado nasce com aviso**, porque o próprio anexo registra que a junta **não fecharia hoje**: os dois primeiros implementadores escreveram envelopes incompatíveis. Nenhuma das quatro jornadas descreve algo que funcione ponta a ponta.
- **Dívida nomeada**: a constituição lista três classes de evidência, e o ADR 0012 criou a quarta. A emenda ao Princípio I não foi feita nesta edição.

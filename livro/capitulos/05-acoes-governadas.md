...
> **Nota datada (2026-08-03, estudo da spec 024)** — o primeiro caso de autor externo ([Traycer](../../estudos/caso-traycer.md)) confirma o gate humano como estado de primeira classe (aprovação pedida/resolvida como eventos tipados, auditoria com ator por evento) e trouxe ao padrão do livro dois desenhos incorporados como DEVERIAs desenhados na [v0.3](https://github.com/GHDaru/protocolos/blob/main/padrao/padrao-aph.md) (APH-5.6/5.7): **gates pendentes sobrevivem à reconexão** — reconectar não perde aprovações aguardando decisão — e **fila de aprovação separada por classe de ação** (edições de arquivo com os caminhos afetados e a operação), o embrião prático de classe de risco vindo de uma implementação que não tem taxonomia. E um empréstimo de apresentação: sumário curto e detalhe da tool call **pré-computados no produtor** — o input bruto nunca vai à tela.

...
- `apps/api/src/ghdaru_api/conversation/domain/catalog.py` — Catálogo v1 com os dois comandos implementados: `ui.navigate` (risk=read; `enum` de rotas derivado dos módulos habilitados do tenant) e `session.logout` (risk=confirm). Lema do arquivo: "o que não está declarado, a IA não faz".
